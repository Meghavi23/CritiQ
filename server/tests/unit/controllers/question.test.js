...
describe('QuestionController', () => {
    ...
    describe('create', () => {
        ...
        it('should call next with 400 if required fields are missing', async () => {
            const next = jest.fn();
            await QuestionController.create(makeReq({ productName: 'Laptop' }), makeRes(), next);
            expect(next).toHaveBeenCalledWith(expect.objectContaining({ statusCode: 400 }));
        });

        it('should call next with 400 if questions is not an array', async () => {
            const next = jest.fn();
            await QuestionController.create(
                makeReq({ ...validBody, questions: 'not-an-array' }),
                makeRes(),
                next
            );
            expect(next).toHaveBeenCalledWith(expect.objectContaining({ statusCode: 400 }));
        ...
        it('should call next with 400 if questions is an empty array', async () => {
            const next = jest.fn();
            await QuestionController.create(
                makeReq({ ...validBody, questions: [] }),
                makeRes(),
                next
            );
            expect(next).toHaveBeenCalledWith(expect.objectContaining({ statusCode: 400 }));
        ...
        it('should set reviewDate when isOrderIdTracking is true', async () => {
        ...
        it('should set excelFile when isOrderIdTracking is false', async () => {
            QuestionService.create.mockResolvedValue({});
            const next = jest.fn();
            const body = {
                ...validBody,
